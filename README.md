...
Install dependencies, validate configuration, apply migrations, and build:
...
`npm start` validates the environment and then runs Next.js on `0.0.0.0:8080`. Put your reverse proxy or platform router in front of that port and set `SELF_URL` to the externally reachable HTTPS origin.
